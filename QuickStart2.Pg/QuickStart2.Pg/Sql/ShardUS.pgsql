...
GRANT USAGE ON SCHEMA "rd" TO "webReader";
GRANT USAGE ON SCHEMA "wt" TO "webWriter";


CREATE FUNCTION shd.shardid()
RETURNS Smallint
AS $$
BEGIN
	RETURN 1;
...
CREATE FUNCTION rd.customerlist()
RETURNS TABLE(customerid int, name varchar(255))
AS $$
...
CREATE FUNCTION rd.customerget(customerid int)
RETURNS SETOF refcursor
AS $$
DECLARE 
	customer_cursor refcursor;
	location_cursor refcursor;
	contact_cursor refcursor;
BEGIN
	OPEN customer_cursor FOR
	SELECT customers.customertypeid, customers.name
	FROM shd.customers
	WHERE customers.customerid = customerid;
	RETURN NEXT customer_cursor;
